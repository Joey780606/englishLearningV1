...
from ui.tab_import_history import ImportHistoryTab
...
class MainWindow(QMainWindow):
    ...
    def _setupUI(self):
        ...
        self._ImportHistoryTab = ImportHistoryTab()
        ...
        self._TabWidget.addTab(self._ImportHistoryTab, "🗂 匯入歷史")
    ...
    def _connectSignals(self):
        """連接跨分頁信號"""
        # Tab 1 完成儲存後，自動刷新詞彙列表與匯入歷史
        self._ImportTab.VocabularySaved.connect(self._VocabTab.refreshTable)
        self._ImportTab.ImportRecordSaved.connect(self._ImportHistoryTab.refreshTable)
        # Tab 1 狀態訊息轉發到狀態列
    ...
    def _onTabChanged(self, Index: int):
        """切換分頁時的處理"""
        if Index == 1:   # 匯入歷史
            self._ImportHistoryTab.refreshTable()
        elif Index == 3:  # 字卡學習
